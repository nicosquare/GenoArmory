...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--tarin_batch_size", default=8, type=int, help="Batch size per GPU/CPU for evaluation.",
    )
    ...
    train_data = prepare_shap_inputs(train_data[:args.tarin_batch_size], device)  # Small sample for background data
    ...
    for i in range(0, test_data.shape[0], args.batch_size):
        x = test_data[i:i + args.batch_size]
        ...
        shap_values.extend(shap_values_i[0])
        pbar.set_description(f'Calculating SHAP values for samples {i+1} to {min(i+args.batch_size, test_data.shape[0])}')
