    ...
    def forward(
        ...
        **kwargs
        ...
        if input_ids is not None and inputs_embeds is not None:
            ...
            input_shape = inputs_embeds.size()[:-1]
            if token_type_ids is None:
                token_type_ids = torch.zeros(input_shape, dtype=torch.long).to(inputs_embeds.device)
            if attention_mask is None:
                attention_mask = torch.ones(input_shape, dtype=torch.long).to(inputs_embeds.device)
