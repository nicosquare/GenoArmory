...
def attack(text_ls, true_label, predictor, word2idx, idx2word, cos_sim, sim_predictor=None,
    ...
    if true_label != orig_label:
        ...
        for idx, synonyms in synonyms_all:
            ...
            if np.sum(new_probs_mask) > 0:
                ...
                semantic_sims_numpy = np.asarray(semantic_sims)  # If it's a scalar or other type, convert it to an array
                threshold_comparison = (semantic_sims_numpy < sim_score_threshold).astype(float)
...
def random_attack(text_ls, true_label, predictor, perturb_ratio, word2idx, idx2word, cos_sim,
    ...
    if true_label != orig_label:
        ...
        for idx, synonyms in synonyms_all:
            ...
            if np.sum(new_probs_mask) > 0:
                ...
                # Convert to PyTorch tensor
                new_label_probs = new_probs[:, orig_label] + threshold_comparison.float()
                new_label_prob_min, new_label_prob_argmin = torch.min(new_label_probs, dim=-1)
                if new_label_prob_min < orig_prob:
                    text_prime[idx] = synonyms[new_label_prob_argmin]
                    num_changed += 1
...
def main():
    ...
    lines = open(args.dataset_path, 'r', encoding='utf-8').readlines()[1:]  # Skip header if present
    texts = []
    labels = []
    ...
    use = USE(model.tokenizer, model.model)
    ...
    for idx, (text, true_label) in enumerate(data):
        if args.perturb_ratio > 0.:
            ...
            new_label, num_queries = random_attack(text, true_label, predictor, args.perturb_ratio,
            ...
            new_text, num_changed, orig_label, \
            new_label, num_queries = attack(text, true_label, predictor,
                                            word2idx, idx2word, cos_sim, sim_predictor=use,
                                            sim_score_threshold=args.sim_score_threshold,
                                            import_score_threshold=args.import_score_threshold,
                                            sim_score_window=args.sim_score_window,
                                            synonym_num=args.synonym_num,
                                            batch_size=args.batch_size, tokenizer=model.dataset.tokenizer)
