...
def main():
    ...
    if args.isMR == 0:
        ...
        try:
            with open(os.path.join(args.original_dataset, 'train.tsv'), "r", encoding="utf-8-sig") as f:
                ...
                for row in data:
                    if row[0] == 'sequence' or row[0] == 'sentence':
                        continue
                    sents.append(row[0])
                    labels.append(row[1])
        except:
            with open(os.path.join(args.original_dataset, 'train.csv'), "r", encoding="utf-8-sig") as f:
                data = csv.reader(f, delimiter=",")
                for row in data:
                    if row[0] == 'sequence' or row[0] == 'sentence':
                        continue
    ...
    try:
        if args.isMR == 1:
            copyfile(os.path.join(args.original_dataset, 'test.csv'), os.path.join(args.output_path, 'test.tsv'))
        else:
            copyfile(os.path.join(args.original_dataset, 'dev.csv'), os.path.join(args.output_path, 'dev.tsv'))
    except: 
        if args.isMR == 1:
