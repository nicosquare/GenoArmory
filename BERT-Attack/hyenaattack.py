...
from transformers import AutoConfig, BertConfig, AutoTokenizer
...
def attack(feature, tgt_model, mlm_model, tokenizer, tokenizer_mlm, k, batch_size, max_length=256, cos_mat=None, w2i={}, i2w={}, use_bpe=1, threshold_pred_score=0.3):
    # MLM-process
    words, sub_words, keys = _tokenize(feature.seq, tokenizer)
    #words = [i for i in words[0]]
    # original label
    inputs = tokenizer.encode_plus(feature.seq, None, add_special_tokens=True, max_length=max_length, pad_to_max_length = True,)
...
def run_attack():
    ...
    config_atk = BertConfig.from_pretrained(mlm_path, trust_remote_code=True)
    ...
    with torch.no_grad():
        for index, feature in enumerate(features[start:end]):
            ...
            feat = attack(feat, tgt_model, mlm_model, tokenizer_tgt, tokenizer_mlm, k, batch_size=32, max_length=256,
