...
def evaluate(features):
    ...
    s = 'origin_acc/aft-atk-acc {:.6f}/ {:.6f}, success4rate {:.4f}, query-num {:.4f}, change-rate {:.4f}'.format(origin_acc, after_atk, success4rate, query, change_rate)
    return s

def dump_features(features, output, s):
    outputs = []
    ...
    with open(output_json, 'a') as f:
        f.write('\n')  # Ensure spacing before appending `s`
        json.dump({'metrics_summary': s}, f, indent=2)
...
def run_attack():
    ...
    s = evaluate(features_output)

    print(data_path)
    print("-------------------------------")

    dump_features(features_output, output_dir, s)
