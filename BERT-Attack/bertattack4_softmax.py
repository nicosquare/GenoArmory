...
def attack(feature, tgt_model, mlm_model, tokenizer, k, batch_size, max_length=512, cos_mat=None, w2i={}, i2w={}, use_bpe=1, threshold_pred_score=0.3, target=None):
    # MLM-process
    ...
    for top_index in list_of_index:
        ...
        for substitute_ in substitutes:
            ...
            if temp_label != orig_label:
                ...
                feature.pred_label = temp_label

                if target == None or (target != None and temp_label == target):
                    feature.success = 4
                    return feature
                else:
                    feature.success = 5
...
def evaluate(features):
    ...
    success5rate = float(success5 / total)
    ...
    print('origin_acc/aft-atk-acc {:.6f}/ {:.6f}, success4rate {:.4f}, success5rate {:.4f}, query-num {:.4f}, change-rate {:.4f}'.format(origin_acc, after_atk, success4rate, success5rate, query, change_rate))
...
def dump_features(features, output):
    ...
    for feature in features:
        outputs.append({'label': feature.label,
                        'pred_label':int(feature.pred_label.item()) if feature.pred_label else None,
...
def run_attack():
    ...
    for layer_idx in range(len(tgt_model.bert.encoder.layer)):
        old_self = tgt_model.bert.encoder.layer[layer_idx].attention.self
        print("----------------------------------------------------------")
        ...
        tgt_model.bert.encoder.layer[layer_idx].attention.self = new_self
    ...
    if args.quantize:
        ...
        tgt_model = QuantizedBertForSequenceClassification(tgt_model, **qparams)
    ...
    with torch.no_grad():
        for index, feature in enumerate(features[start:end]):
            ...
            feat = attack(feat, tgt_model, mlm_model, tokenizer_tgt, k, batch_size=32, max_length=128,
                          cos_mat=cos_mat, w2i=w2i, i2w=i2w, use_bpe=use_bpe,threshold_pred_score=threshold_pred_score, target=target)
