...
MODEL_CLASSES = {
    ...
    "hyena": (AutoConfig, AutoModelForSequenceClassification, AutoTokenizer),
    "nt": (AutoConfig, AutoModelForSequenceClassification, AutoTokenizer),
    "og": (AutoConfig, AutoModelForSequenceClassification, AutoTokenizer),
}
...
def train(args, train_dataset, model, tokenizer, experiment=None):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.model_type in ["dnabert", "nt", "og"]:
            ...
            if args.adv_init_mag > 0:
                if args.model_type in ["dnabert", "nt", "og"]:
                    ...
                    input_mask = torch.ones_like(embeds_init)
                    input_lengths = torch.sum(input_mask, 1)
                    if args.norm_type == "l2":
                        delta = torch.zeros_like(embeds_init).uniform_(
                            -1, 1
