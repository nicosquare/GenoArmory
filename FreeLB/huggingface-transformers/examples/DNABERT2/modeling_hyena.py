...
class HyenaOperator(nn.Module):
    def __init__(
        ...
        self.p_dropout = config.hyena_dropout
    ...
    def forward(self, u, dp_mask=None):
        l = u.size(-2)
        ...
        *x, v = uc.split(self.d_model, dim=1)
        ...
        if self.p_dropout > 0.0 and self.training:
                if dp_mask is None:
                    mask1 = torch.zeros_like(y).bernoulli_(1 - self.p_dropout) / (1 - self.p_dropout)
                else:
                    mask1 = dp_mask
                y = mask1 * y
        else:
...
class HyenaBlock(nn.Module):
    ...
    def forward(self, hidden_states, dp_masks=None):
        ...
        hidden_states, mask1 = self.mixer(hyena_normed, dp_mask=dp_masks)
        dp_mask_ = mask1
        ...
        return hidden_states + residual, dp_mask_
...
class HyenaLMBackbone(nn.Module):
    ...
    def forward(self, input_ids, inputs_embeds=None, output_hidden_states=False, dp_masks=None):
        all_hidden_states = []
        ...
        for i in range(len(self.layers)):
            ...
            if self.gradient_checkpointing and self.training:
                hidden_states, dp_mask_ = self._gradient_checkpointing_func(layer.__call__, hidden_states, dp_mask=dp_masks[i] if dp_masks is not None else None,)
            else:
                hidden_states, dp_mask_ = layer(hidden_states, dp_masks=dp_masks[i] if dp_masks is not None else None)
            if output_hidden_states:
                all_hidden_states.append(hidden_states)
            ret_dp_masks.append(dp_mask_)
        hidden_states = self.ln_f(hidden_states.to(dtype=self.ln_f.weight.dtype))
        if output_hidden_states:
            all_hidden_states.append(hidden_states)

        return hidden_states, all_hidden_states, ret_dp_masks
...
class HyenaDNAModel(HyenaDNAPreTrainedModel):
    ...
    def forward(self, input_ids, inputs_embeds=None, output_hidden_states=None, return_dict=None, dp_masks=None):
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        )
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict

        hidden_states, all_hidden_states, dp_mask = self.backbone(input_ids, inputs_embeds=inputs_embeds, output_hidden_states=output_hidden_states, dp_masks=dp_masks)
        if return_dict:
            return BaseModelOutputWithNoAttention(last_hidden_state=hidden_states,
                                                  hidden_states=all_hidden_states if output_hidden_states else None), dp_mask
        elif output_hidden_states:
            return hidden_states, all_hidden_states, dp_mask
        else:
            return hidden_states, None, dp_mask
...
class HyenaDNAForSequenceClassification(HyenaDNAPreTrainedModel):
    ...
    def forward(
        ...
        dp_masks: Optional[torch.Tensor] = None,
        ...
        transformer_outputs, encoder_dp_masks = self.hyena(
            input_ids,
        ...
        if self.p_dropout > 0 and self.training:
            if dp_masks is None:
                mask1 = torch.zeros_like(logits).bernoulli_(1 - self.p_dropout) / (1 - self.p_dropout)
            else:
                mask1 = dp_masks[0]
            logits = mask1 * logits
        else:
        ...
        if self.training:
            return outputs, [mask1, encoder_dp_masks]  # (loss), logits, (hidden_states), (attentions)
        else:
            return outputs