...
class BertUnpadSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, cu_seqlens: torch.Tensor,
                ...
                attn_mask: torch.Tensor, bias: torch.Tensor, dp_mask:torch.Tensor) -> torch.Tensor:
        ...
        if self.p_dropout or flash_attn_qkvpacked_func is None:
            ...
            if self.p_dropout > 0 and self.training:
                if dp_mask is None:
                    mask1 = torch.zeros_like(attention_probs).bernoulli_(1 - self.p_dropout) / (1 - self.p_dropout)
                else:
                    mask1 = dp_mask
                attention_probs = mask1 * attention_probs
            else:
        ...
        return rearrange(attention, 'nnz h d -> nnz (h d)'), mask1
...
class BertUnpadAttention(nn.Module):
    ...
    def forward(
        ...
        """
        self_output, mask1 = self.self(input_tensor, cu_seqlens, max_s, indices,
                                attn_mask, bias, dp_mask)
        if subset_idx is not None:
            return self.output(index_first_axis(self_output, subset_idx),
                               index_first_axis(input_tensor, subset_idx)), mask1
        else:
            return self.output(self_output, input_tensor), mask1
...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        attention_output, mask1 = self.attention(hidden_states, cu_seqlens, seqlen,
                                          subset_idx, indices, attn_mask, bias, dp_mask=dp_mask)
        dp_mask_ = mask1
        layer_output = self.mlp(attention_output)
        return layer_output, dp_mask_
    ...
    def forward(
        ...
        if subset_mask is None:
            for i in range(len(self.layer)):
                ...
                hidden_states, dp_mask_ = layer_module(hidden_states,
                                             cu_seqlens,
                                             seqlen,
                                             None,
                                             indices,
                                             ...
                                             dp_mask=dp_masks[i] if dp_masks is not None else None
                                             )
        ...
        if not output_all_encoded_layers:
            all_encoder_layers.append(hidden_states)
        return all_encoder_layers, ret_dp_masks
    ...
    def forward(
        ...
        **kwargs
        ...
        if input_ids is None:
            ...
            if token_type_ids is None:
                token_type_ids = torch.zeros_like(inputs_embeds[:,:,0]).long()
        else:
            if attention_mask is None:
        ...
        encoder_outputs, dp_mask = self.encoder(
            embedding_output,
            attention_mask,
        ...
        if self.pooler is not None:
            return encoder_outputs, pooled_output, dp_mask

        return encoder_outputs, None, dp_mask
...
class DNABertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(
        ...
        if self.p_dropout > 0 and self.training:
            if dp_masks is None:
                mask1 = torch.zeros_like(pooled_output).bernoulli_(1 - self.p_dropout) / (1 - self.p_dropout)
            else:
                mask1 = dp_masks[0]
