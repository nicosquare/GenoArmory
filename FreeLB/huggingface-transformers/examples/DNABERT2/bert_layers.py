    ...
    def forward(
        ...
        if self.p_dropout or flash_attn_qkvpacked_func is None:
            ...
            if self.p_dropout > 0 and self.training:
                if dp_mask is None:
                    mask1 = torch.zeros_like(attention_probs).bernoulli_(
                        1 - self.p_dropout
                    ) / (1 - self.p_dropout)
                else:
                    mask1 = dp_mask
...
class BertUnpadAttention(nn.Module):
    ...
    def forward(
        ...
        """
        self_output, mask1 = self.self(
            input_tensor, cu_seqlens, max_s, indices, attn_mask, bias, dp_mask
        )
...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        attention_output, mask1 = self.attention(
            hidden_states,
            cu_seqlens,
            seqlen,
            subset_idx,
    ...
    def forward(
        ...
        if subset_mask is None:
            for i in range(len(self.layer)):
                ...
                hidden_states, dp_mask_ = layer_module(
                    hidden_states,
                    cu_seqlens,
                    seqlen,
                    None,
    ...
    def forward(
        ...
        **kwargs,
        ...
        embedding_output = self.embeddings(
            input_ids, token_type_ids, position_ids, inputs_embeds=inputs_embeds
        )
...
class DNABertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(
        ...
        if self.p_dropout > 0 and self.training:
            if dp_masks is None:
                mask1 = torch.zeros_like(pooled_output).bernoulli_(
                    1 - self.p_dropout
                ) / (1 - self.p_dropout)
            else:
                mask1 = dp_masks[0]
