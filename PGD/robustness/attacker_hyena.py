...
class AttackerHyena(ch.nn.Module):
    """
    ...
    def __init__(self, args, model, dataset):
        ...
        """
        super(AttackerHyena, self).__init__()
        ...
        def get_adv_examples(x):
            ...
            def replace_best(loss, bloss, x, bx):
                if bloss is None:
                    ...
                    replace = replace.to(x["labels"].device)
            ...
            for _ in iterator:
                ...
                losses, out = calc_loss({"inputs_embeds": step.to_image(inputs_embeds).to(self.model.device),
                                 "labels": x["labels"].to(self.model.device)}, target.to(self.model.device))
...
class AttackerHyenaModel(ch.nn.Module):
    """
    ...
    """
    def __init__(self, args, model, dataset):
        super(AttackerHyenaModel, self).__init__()
        self.normalizer = self.normalize
        self.model = model
        self.attacker = AttackerHyena(args, model, dataset)
        self.args = args
    ...
    def forward(self, inp, target=None, make_adv=False, with_latent=False,
        ...
        output = self.model(inputs_embeds=normalized_inp['inputs_embeds'], labels=normalized_inp['labels'])
        if with_image:
            return (output, inp)
        return output
